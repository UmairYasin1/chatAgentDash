...
const Header =({setWebname,webname})=>{
 ...
 useEffect (()=>{
  axios
  .get('http://173.254.252.226:5001/brand/allbrands')
  .then((res)=>{
...
//  console.log('curVisObj ~~~~~~~~~~~~', curVisObj);
  ...
  //console.log('jiiiii bhaiii$$$$', curVisObj, data, roomId);

 // var sendData = { currentVis : curVisObj, currentData : data };
  // socket.emit('get-current-visitor-response', sendData, function (response) {
  //   console.log(response,"get-current-visitor-response");
  // });
  // console.log('1111',sendData.currentVis);
  // console.log('2222',sendData.currentData);
  //socket.emit('get-current-visitor-response', sendData);
