...
const Messagereducer = (messages, action) => {
  ...
  //  alert("customer id" + action.payload.visitorId)
      ...
      case "forLastMessage":
          ...
          let agent = messages.filter( x => x.repmsgFrom === action.payload ).length -1 
            // messages[agent].repmsg
      ...
      //  console.log("index",index1)
             if(~index1)  
              ...
              //repIsRead: false
              repIsRead: action.payload.isRead
              }
             else 
   ...
   case "GET_RESPONSE" :
  let index3 = messages.findIndex( x => x.msgId === action.payload.msgId )
      // console.log("index",index)
         if(~index3)
   ...
   socket.on('chat-msg',function(data){
    ...
    if(data.msg !== ""){
      if(data.file !== "")
      {
        ...
        {
          socket.emit('msg-is-read',{ visitor_id:visitorIdVal, msgId: data.id});
          Messagedispatch({
    ...
    let roomId = '';
        ...
        socket.on('update-room',function(room){
           console.log(room,"update-rommmmms")
          const msgCount = 0;
          roomId = room;
      ...
      //     console.log(room,"update-room-unassigned", room)
      //    const msgCount = 0;
      //    roomId = room;
      //    socket.emit('old-chats-init',{room:roomId,username:obj.data.agent_id,msgCount:msgCount});
          ...
          if(data.room === roomId ){
            if(data.result.length !== 0){
              for (var i = 0;i < data.result.length;i++) 
              {
                  ...
                  {
                      // console.log(response,"get-reply-response")
                      Messagedispatch({
                        type: "GET_RESPONSE",
                        payload: response
                      });
